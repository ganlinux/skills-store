...
- **Check for updates**: Read `~/.local/bin/.skills-store/last_check_memepump_scanner` and compare with current timestamp:
  ```bash
  cached_ts=$(cat ~/.local/bin/.skills-store/last_check_memepump_scanner 2>/dev/null || true)
  now=$(date +%s)
  ```
